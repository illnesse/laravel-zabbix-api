...
abstract class ZabbixApiAbstract
{
    ...
    public function __construct($apiUrl, $user, $password, $httpUser, $httpPassword, $authToken, $sslContext, $checkSsl)
    {
        ...
        if ($authToken) {
            ...
            // Try with 'username' (Zabbix 5.4+), fallback to 'user' (older versions)
            try {
                $this->userLogin(array('username' => $user, 'password' => $password));
            } catch (Exception $e) {
                // If it fails with invalid parameter error, try old 'user' parameter
                if (strpos($e->getMessage(), 'unexpected parameter') !== false) {
    ...
    final public function userLogin($params=array(), $arrayKeyProperty='', $tokenCacheDir='/tmp')
    {
        ...
        // Support both 'username' (Zabbix 5.4+) and 'user' (older versions)
        $userParam = isset($params['username']) ? $params['username'] : (isset($params['user']) ? $params['user'] : null);
        if ($tokenCacheDir && $userParam && is_dir($tokenCacheDir)) {
            $tokenCacheFile = $tokenCacheDir.'/.zabbixapi-token-'.md5($userParam.'|'.posix_getuid());
        }
